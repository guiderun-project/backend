...
public class EventLikeController {
    ...
    public ResponseEntity<LikeCountResponse> pressCommentLike(@PathVariable Long commentId, HttpServletRequest request){
    ...
    public ResponseEntity<LikeCountResponse> getCommentLikeCount(@PathVariable Long commentId, HttpServletRequest request){
        String privateId = jwtProvider.extractUserId(request);
        return ResponseEntity.ok().body(eventLikeService.getCommentLikeCount(commentId));
    }
    @PostMapping("/{eventId}/likes")
    public ResponseEntity<LikeCountResponse> pressEventLike(@PathVariable Long eventId, HttpServletRequest request){
        String privateId = jwtProvider.extractUserId(request);
    ...
    public ResponseEntity<LikeCountResponse> getEventLikeCount(@PathVariable Long eventId, HttpServletRequest request){
        String privateId = jwtProvider.extractUserId(request);
        return ResponseEntity.ok().body(eventLikeService.getEventLikeCount(eventId));
    }
}
